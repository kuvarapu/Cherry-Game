...
// Helper function to update board with current player positions
function updateBoardWithPositions(game) {
  // Reset board to cherry spaces only
  game.board = new Array(100).fill(null);
  const cherrySpaces = [14, 29, 44, 59, 74, 89]; // 0-indexed positions for 15, 30, 45, 60, 75, 90
  cherrySpaces.forEach(pos => game.board[pos] = 'cherry');
  
  // Add player positions (convert from 1-indexed to 0-indexed)
  if (game.positions[0] && game.positions[0] > 0 && game.positions[0] <= 100) {
...
router.post('/game', async (req, res) => {
  try {
    ...
    console.log('🎯 Creating game with mode:', mode, 'username:', username);
    
    if (!mode || !username) {
      console.log('❌ Missing required fields - mode:', mode, 'username:', username);
      return res.status(400).json({ error: 'Mode and username are required' });
    }
    
    // Map frontend mode names to backend expected values
    const gameMode = mode === 'single' ? 'pvc' : 'pvp';
    console.log('🎯 Mapped game mode:', gameMode);
    
    if (!['pvp', 'pvc'].includes(gameMode)) {
      console.log('❌ Invalid game mode:', gameMode);
      return res.status(400).json({ error: 'Mode must be either "single" or "multi"' });
    }
    
    const players = gameMode === 'pvp' ? [username, 'Player2'] : [username, 'Computer'];
    console.log('👥 Players:', players);
    
    // Initialize game board (100 cells, cherry spaces at positions 15, 30, 45, 60, 75, 90)
...
// Alternative endpoint for game creation (frontend compatibility)
router.post('/game/start', async (req, res) => {
  try {
    const { mode, username } = req.body;
    
    console.log('🎯 Creating game at /game/start with mode:', mode, 'username:', username);
...
// Get game by ID with enhanced error handling
...
router.post('/game/:id/roll', async (req, res) => {
  try {
    ...
    if (!game.finished && game.mode === 'pvc' && game.turn === 1) {
      // Use setTimeout to make computer move after response is sent
      setTimeout(async () => {
        try {
...
// Move endpoint (alias for roll - frontend compatibility)
router.post('/:id/move', async (req, res) => {
  try {
    ...
    let newPosition = fromPosition + dice;
    let cherryBonus = false;

    // Handle cherry spaces (every 15th space: 15, 30, 45, 60, 75, 90)
    if (newPosition % 15 === 0 && newPosition < 100) {
      cherryBonus = true;
      newPosition += 5; // Cherry bonus
    }
    ...
    // Switch turns if game isn't finished
    if (!game.finished) {
    ...
    // If vs computer and it's computer's turn, make AI move automatically
    if (!game.finished && game.mode === 'pvc' && game.turn === 1) {
      setTimeout(async () => {
        try {
          await makeComputerMove(game);
    ...
    res.status(500).json({ error: 'Failed to make move. Please try again.' });
  }
});
